# Registers the Autohotkey language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koAutohotkeyLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Autohotkey")
    registry.registerLanguage(KoAutohotkeyLanguage())

class KoAutohotkeyLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Autohotkey"
    lexresLangName = "Autohotkey"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "a2b27bca-43ea-4ba7-abf5-eeb3b7c2288e"
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Autohotkey'}
