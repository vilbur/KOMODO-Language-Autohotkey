...
"""Autohotkey support for codeintel.
...
lang = "Autohotkey"
log = logging.getLogger("codeintel.autohotkey")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Autohotkey-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
]
...
# lexers. You must define a Lexer class for lang Autohotkey. If Komodo's
# scintilla lexer for Autohotkey is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class AutohotkeyLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Autohotkey is one of Komodo's existing C++ lexers
...
# examples of usage.
class AutohotkeyLangIntel(LangIntel):
    lang = lang
...
#
class AutohotkeyBuffer(UDLBuffer):
...
# Autohotkey content and returns a description of the code in that file.
# See "cile_autohotkey.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Autohotkey is
...
class AutohotkeyCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_autohotkey
        return cile_autohotkey.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=AutohotkeyLexer(),
        buf_class=AutohotkeyBuffer,
        langintel_class=AutohotkeyLangIntel,
        import_handler_class=None,
        cile_driver_class=AutohotkeyCILEDriver,
